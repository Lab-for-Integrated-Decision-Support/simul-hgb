...
The NNM figure is created from a manually-entered data frame of values, because we do not save the NNM (see ~ line 939 in `03_Clinical_Accuracy.Rmd`).
...
Here we plot figures necessary for manuscript revision.
...
ggplot2::ggsave(
  filename = file.path(
    Sys.getenv('PICU_LAB_IMG_PATH'),
    paste0('Supp-Figure-TimeThreshold_', run.date, ext)
  ),  
  plot = Supp_Figure_TimeThresh,
  width = 12, height = 4, units = 'in', dpi = dpi)
```
...
The variable of interest is `ba.all.non.param`:
...
And create the 2x2 figure:

```{r}
Supp_Figure_NonParamBA <- 
  cowplot::plot_grid(
    CHOP.BA.NP.BG$qq.plot + 
      ggtitle('CHOP BG Q:Q Plot') +
      theme(
        #axis.title.x = element_blank()
      ),
    URMC.BA.NP.BG$qq.plot + 
      ggtitle('URMC BG Q:Q Plot') +
      theme(
        #legend.position = 'none'
      ),
    CHOP.BA.NP.BG$plot +
      ggtitle('CHOP B-A Plot') +
      theme(
        #axis.title.x = element_blank()
      ),
    URMC.BA.NP.BG$plot +
      ggtitle('URMC B-A Plot') +
      theme(
        #axis.title.x = element_blank()
      ),    
...
ggplot2::ggsave(
  filename = file.path(
    Sys.getenv('PICU_LAB_IMG_PATH'),
    paste0('Supp-Figure-NonParam-BA_', run.date, ext)
  ),  
  plot = Supp_Figure_NonParamBA,
  width = 8, height = 8, units = 'in', dpi = dpi)
```

```{r, echo=FALSE}
rm(CHOP.BA.NP.BG, URMC.BA.NP.BG)
...
3 x 1 panel of CHOP BG, POC and URMC BG displaying distributions along with text describing data

##### B-A Analysis

3 x 1 panel of CHOP BG, POC, and URMC BG showing B-A plots along with text describing bias and LOAs

##### Time TO Result

2 x 1 panel of CHOP BG, URMC BG for Time TO Result along with text describing TTR
